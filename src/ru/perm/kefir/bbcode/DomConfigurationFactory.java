...
public class DomConfigurationFactory {
    ...
    public Configuration create(Document dc) {
        ...
        try {
            ...
            // Parse codes
            Map<String, ru.perm.kefir.bbcode.configuration.Code> codes = parseCodes(dc);
    ...
    private Pattern parsePattern(Node node) {
        ...
        return new ru.perm.kefir.bbcode.configuration.Pattern(elements);
    }
    ...
    private ru.perm.kefir.bbcode.configuration.Text parseText(
            Node el,
            String name
    ) {
        ru.perm.kefir.bbcode.configuration.Text text;
        if (nodeAttribute(el, TAG_VAR_ATTR_INHERIT, DEFAULT_INHERIT_VALUE)) {
            text = new ru.perm.kefir.bbcode.configuration.Text(
                    name, null,
    ...
    private ru.perm.kefir.bbcode.configuration.Variable parseVariable(Node el, String name) {
        ru.perm.kefir.bbcode.configuration.Variable variable;
        if (nodeHasAttribute(el, TAG_VAR_ATTR_REGEX)) {
            variable = new ru.perm.kefir.bbcode.configuration.Variable(
                    name,
            ...
            variable = new ru.perm.kefir.bbcode.configuration.Variable(name);
        }
        return variable;
    }
